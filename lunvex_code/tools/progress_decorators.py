...
from ..progress import spinner, progress_bar, multi_progress, ProgressConfig
...
def with_progress(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(self, *args, **kwargs) -> Any:
            # Determine message
            progress_message = message or getattr(self, 'name', func.__name__).replace('_', ' ').title()
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(self, *args, **kwargs) -> Any:
            ...
            with spinner(progress_message) as progress:
                try:
                    result = func(self, *args, **kwargs)
                    progress.stop(f"✓ Found {len(result.output.splitlines()) if hasattr(result, 'output') else 'results'}", success=True)
                    return result
                except Exception as e:
                    progress.stop(f"✗ {progress_message}: {str(e)}", success=False)
                    raise
...
def with_git_progress(message: Optional[str] = None) -> Callable:
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(self, *args, **kwargs) -> Any:
            ...
            progress_message = message or f"Git {getattr(self, 'name', func.__name__).replace('_', ' ')}"
