...
class TaskPlanner:
    ...
    def _is_complex_task(self, task: str) -> bool:
        ...
        file_mentions = sum(
            len(re.findall(pattern, task_lower, re.IGNORECASE))
        ...
        for subtask_id in plan.execution_order:
            ...
            for dep_id in subtask.dependencies:
                ...
                if not dep or not dep.completed:
                    ui.print_error(f"Cannot execute {subtask_id}: dependency {dep_id} not completed")
    ...
    def _update_context_from_result(self, subtask: Subtask, result: str, shared_context: Dict[str, Any]):
        """
    ...
    def _generate_final_summary(self, plan: TaskPlan) -> str:
        ...
        # Final result
        summary_parts.append("\n## Final Result")
        summary_parts.append("All subtasks completed successfully." if plan.completed else "Task failed to complete.")
        
        return "\n".join(summary_parts)


def create_task_planner(client: LunVexClient, tool_registry: ToolRegistry, working_dir: str = ".") -> TaskPlanner:
