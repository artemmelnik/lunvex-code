...
This guide shows you how to create demos and screencasts for LunVex Code.

本指南向您展示如何为 LunVex Code 创建演示和屏幕录制。
...
```bash
# Run a single task
lunvex-code run "Add type hints to main.py"
```
...
lunvex-code run --trust "Refactor the authentication module"
```
...
lunvex-code run --yolo "Generate a complete REST API"
```
...
# Show Trust mode
lunvex-code run --trust "Create a new file"

# Show YOLO mode
lunvex-code run --yolo "Generate a complete module"
```
...
1. Open an issue at https://github.com/artemmelnik/lunvex-code/issues
