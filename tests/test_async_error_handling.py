...
async def test_async_read_file_permission_denied():
    ...
    assert any(word in error_lower for word in ["error", "permission", "directory", "access", "denied", "cannot"])
...
async def test_async_write_file_invalid_path():
    ...
    # Different systems may give different error messages
    error_lower = result.error.lower()
    assert any(word in error_lower for word in ["error", "permission", "access", "denied", "cannot", "failed"])
...
async def test_async_bash_dangerous_command():
    ...
    for cmd in dangerous_commands:
        ...
        assert not result.success
        assert "blocked" in result.error.lower() or "dangerous" in result.error.lower() or "not allowed" in result.error.lower()
...
async def test_async_tool_concurrent_errors():
    ...
    tasks = [
        tool.execute(path=f"/nonexistent/file{i}.txt")
        for i in range(5)
    ]
