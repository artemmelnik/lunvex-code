    ...
    try:
        ...
        with patch.object(tool, "_update_progress") as mock_update:
            ...
            if os.path.getsize(tmp_path) > 1024 * 1024:
                ...
                print(
                    f"  Note: File size is {os.path.getsize(tmp_path)} bytes, progress may not trigger"
                )
...
def test_edit_file_with_progress():
    ...
    try:
        ...
        # Mock progress update
        with patch.object(tool, "_update_progress") as mock_update:
            result = tool.execute(tmp_path, old_str="Line 2", new_str="Line 2 edited")
...
def test_grep_with_progress():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Mock progress update
        with patch.object(tool, "_update_progress") as mock_update:
            result = tool.execute(pattern="pattern", path=tmpdir, include="*.txt")
...
def test_progress_configuration():
    ...
    config = ProgressConfig(spinner_style="red", progress_style="yellow", transient=False)
