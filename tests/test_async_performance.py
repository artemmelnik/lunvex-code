...
async def test_async_tools_parallel_performance():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        start_time = time.time()
        tasks = [tool.execute(path=str(tmpdir_path / f"file{i}.txt")) for i in range(min(10, num_files))]
...
async def test_async_system_scalability():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for level in concurrency_levels:
            ...
            tasks = [tool.execute(path=str(tmpdir_path / f"test_{level}_{i}.txt")) 
                    for i in range(level)]
