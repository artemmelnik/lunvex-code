...
class TestPermissionManager:
    ...
    def test_default_mode_edit_file_asks(self):
        ...
        manager = PermissionManager()
        request = manager.check_permission(
            "edit_file", {"path": "/some/file.py", "old_str": "...", "new_str": "..."}
        )
        assert request.level == PermissionLevel.ASK
    ...
    def test_bash_command_permission_levels(self):
        """Test that bash commands have appropriate permission levels in default mode."""
        manager = PermissionManager()

        # Commands that match safe patterns should be auto-approved
        ...
        for cmd in auto_approved_commands:
            request = manager.check_permission("bash", {"command": cmd})
            assert request.level == PermissionLevel.AUTO, f"Command {cmd} should be auto-approved"

        # Commands that don't match safe patterns should ask for permission
        ask_commands = [
            "python main.py",
            "rm file.txt",
            "touch newfile.txt",
        ...
        for cmd in ask_commands:
            request = manager.check_permission("bash", {"command": cmd})
            assert request.level == PermissionLevel.ASK, f"Command {cmd} should ask for permission"
