...
from lunvex_code.llm_cache import LLMCache, configure_llm_cache, get_llm_cache
...
def test_llm_cache_basic():
    ...
    cache.put(model="test-model", messages=messages, response=response_data, token_count=10)
...
def test_llm_cache_lru_eviction():
    ...
    cache.put(model="test-model", messages=messages3, response=response_data, token_count=10)
...
def test_llm_cache_invalidation():
    ...
    # cache_keys = list(cache.cache.keys())  # Unused variable
...
def test_llm_cache_with_tools():
    ...
    cache.put(
        model="test-model", messages=messages, response=response_data, tools=tools, token_count=15
    )
