...
async def test_async_edit_file_tool():
    ...
    try:
        ...
        result = await tool.execute(
            path=temp_file, old_str="Original line 2", new_str="Modified line 2"
        )
...
async def test_async_grep_parallel_search():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for i in range(20):
            (tmpdir_path / f"file{i}.txt").write_text(
                f"This is file {i}\nSearch term here\nMore content"
            )
        ...
        result = await tool.execute(pattern="Search term", path=tmpdir, include="*.txt", limit=100)
