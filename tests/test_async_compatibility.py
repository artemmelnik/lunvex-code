...
async def test_sync_async_equivalent_results():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        async_write_result = await async_write.execute(
            path=str(new_file.with_suffix(".async.txt")), content="Async write"
        )
        ...
        sync_edit_result = sync_edit.execute(
            path=str(edit_file), old_str="Original text", new_str="Modified text"
        )
        ...
        async_edit_result = await async_edit.execute(
            path=str(async_edit_file), old_str="Original text", new_str="Modified text"
        )
...
async def test_mixed_sync_async_workflow():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        result = await async_grep.execute(
            pattern="text to search", path=str(tmpdir_path), include="*.txt"
        )
        ...
        tasks = [
            async_read.execute(path=str(tmpdir_path / filename)) for filename in files_to_create
        ]
...
if __name__ == "__main__":
    ...
    async def run_all_tests():
        ...
        print(f"\n{'=' * 60}")
        print("Running: test_tool_registry_compatibility")
        print("=" * 60)
        try:
            test_tool_registry_compatibility()
            print("✅ test_tool_registry_compatibility PASSED")
            passed += 1
        except Exception as e:
