...
async def test_smoke_async_write_and_read():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        from lunvex_code.tools.async_file_tools import AsyncWriteFileTool, AsyncReadFileTool
...
async def test_smoke_async_parallel_execution():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Create tasks
        tasks = [
            tool.execute(path=str(tmpdir_path / f"parallel_{i}.txt"))
            for i in range(num_files)
        ]
...
if __name__ == "__main__":
    ...
    async def run_all_smoke_tests():
        ...
        try:
            ...
            print(f"✅ test_smoke_sync_async_compatibility: PASSED")
            passed += 1
        except Exception as e:
