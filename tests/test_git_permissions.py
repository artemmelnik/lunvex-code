...
from lunvex_code.permissions import PermissionLevel, PermissionManager
...
def test_git_tools_permission_rules():
    ...
    for tool_name in modifying_tools:
        ...
        assert (
            "modif" in request.reason.lower()
            or "creates" in request.reason.lower()
            or "updates" in request.reason.lower()
        )
...
def test_git_tools_with_trust_mode():
    ...
    assert (
        request.level == PermissionLevel.AUTO
    ), "git_branch should be AUTO in trust mode (auto-approved)"
...
def test_git_tools_with_yolo_mode():
    ...
    assert (
        request.level == PermissionLevel.AUTO
    ), "git_branch should be AUTO in YOLO mode (auto-approved)"
