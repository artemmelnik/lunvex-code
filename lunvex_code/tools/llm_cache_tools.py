...
from ..llm_cache import get_llm_cache, configure_llm_cache, save_llm_cache
...
class ClearLLMCacheTool(Tool):
    ...
    def execute(self, **kwargs) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(
                success=True,
                output="✅ LLM cache cleared successfully"
            )
        except Exception as e:
            return ToolResult(
                success=False,
                output="",
                error=f"Failed to clear LLM cache: {str(e)}"
            )
...
class ConfigureLLMCacheTool(Tool):
    ...
    def execute(self, **kwargs) -> ToolResult:
        ...
        try:
            ...
            if ttl_seconds is not None:
                output_lines.append(f"TTL: {current_ttl}s → {ttl_seconds}s ({ttl_seconds/3600:.1f} hours)")
            ...
            return ToolResult(
                success=False,
                output="",
                error=f"Failed to configure LLM cache: {str(e)}"
            )
    ...
    def execute(self, **kwargs) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(
                success=False,
                output="",
                error=f"Failed to invalidate LLM cache: {str(e)}"
            )


# Import time module for the stats tool
import time