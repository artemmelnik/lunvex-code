...
async def test_git_log_async_context():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        subprocess.run(["git", "add", "."], cwd=tmpdir, capture_output=True)
        subprocess.run(["git", "commit", "-m", "Add file 1"], cwd=tmpdir, capture_output=True)

        (tmpdir_path / "file2.txt").write_text("File 2")
        subprocess.run(["git", "add", "."], cwd=tmpdir, capture_output=True)
        subprocess.run(["git", "commit", "-m", "Add file 2"], cwd=tmpdir, capture_output=True)
...
async def test_git_tools_concurrent():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        subprocess.run(["git", "add", "."], cwd=tmpdir, capture_output=True)
        subprocess.run(["git", "commit", "-m", "Initial files"], cwd=tmpdir, capture_output=True)

        # Import git tools
        from lunvex_code.tools.git_tools import GitLogTool, GitStatusTool
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if not result.success:
            ...
            assert (
                ...
                or "fatal" not in result.output.lower()
            )
...
async def test_git_show_async_context():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        result = subprocess.run(
            ["git", "commit", "-m", "Special commit"], cwd=tmpdir, capture_output=True, text=True
        )
        ...
        result = await asyncio.to_thread(
            lambda: git_show.execute(object=commit_hash[:8], path=tmpdir)
        )
